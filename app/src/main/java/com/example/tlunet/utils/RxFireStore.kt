        ...
        val listener = addSnapshotListener { documentSnapshot, firebaseFirestoreException ->
            documentSnapshot?.let {
                if (documentSnapshot.exists()) {
                    try {
                        documentSnapshot.toObject(T::class.java)?.let { it1 -> emitter.onNext(it1) }
                    } catch (e: Exception) {
                        emitter.onError(e)
                    }
                } else {
    ...
    return Single.create { emitter ->
        get()
                .addOnSuccessListener {
                    if (it.exists()) {
                        try {
                            it.toObject(T::class.java)?.let { it1 -> emitter.onSuccess(it1) }
                        } catch (e: Exception) {
                            emitter.onError(e)
                        }
                    } else {
...
/**
 * Increment's a value at the given [DocumentReference] (receiver) by [increment] and returns a
 * Single that emits the new value if the [Transaction] completes successfully or calls onError
 * otherwise
 *
 * @param fieldName of the field to be incremented
 * @param increment number to increment the field by
 *
 * @receiver [DocumentReference] to update
 */
fun DocumentReference.incrementField(fieldName: String, increment: Long = 1): Single<Long> {
    return FirebaseFirestore.getInstance()
            .runTransactionSingle(Transaction.Function {
                ...
                val newValue = docSnapshot.getLong(fieldName)?.plus(increment)
